...
class GrowthCampaign(TutorialHarness):
    ...
    def validate_step(self, v_traces: Dict[str, np.ndarray], model: GrowthModel):
        ...
        # Physiological (minimal activity check)
        # sensory/match should have more spikes than baseline
        def get_spikes(v):
            return np.sum((v[:, :-1] < 0) & (v[:, 1:] >= 0))
            
        spikes = {c: get_spikes(v) for c, v in v_traces.items()}
    ...
    def run_falsifier(self, model: GrowthModel):
        ...
        if np.isnan(v_sensory).any() or np.isinf(v_sensory).any():
            reasons.append("NaN/Inf under stress input")
        
        if np.max(v_sensory) > 100:
            reasons.append(f"Runaway excitation under stress: max V = {np.max(v_sensory):.2f}")
            
        # 2. No-input recovery test
        v_baseline = stress_model.simulate("baseline")
        # Check if it settles back to -65ish
                ...
                if f_passed:
                    ...
                    if prev_n >= 4:
                        ...
                        if prev_n == 4:
                            ...
                            cp_path = os.path.join(self.artifact_dir, f"checkpoint_N{prev_n}.json")
                            with open(cp_path) as f:
                                model = GrowthModel.from_dict(json.load(f), self.mission_config)
                        consecutive_failures += 1
                    else:
                        logger.error("Falsifier failed at first checkpoint. Stopping.")
                        break
