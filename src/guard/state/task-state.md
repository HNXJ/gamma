...
1. Ensure `train_lora_mlx.py` is running (or start it if needed, redirecting output to `training.log`).
...
2. Execute `pgrep -f train_lora_mlx.py` to isolate the Process ID (PID).
...
- Start command: `python3 scripts/train_lora_mlx.py > training.log 2>&1 &`
